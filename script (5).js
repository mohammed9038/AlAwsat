...
function updateProgress() {
  ...
  // Check if categories have quantities
  const hasCategories = Object.values(appState.productQuantities).some(quantities => {
    return quantities.qty > 0 || quantities.sellout > 0;
  });
  const categoryStep = document.querySelector('[data-step="5"]');
  
  if (hasCategories) {
    categoryStep.classList.add('completed');
    completedSteps++;
  } else {
    categoryStep.classList.remove('completed');
    if (completedSteps === 4) {
...
async function loadDropdowns() {
  try {
    ...
    showToast('Unable to load dropdown data. Please check your internet connection.', 'warning', 'Data Loading');
    hideLoadingIndicator('channel');
    
    // Set default options when data fails to load
    elements.channelSelect.innerHTML = '<option value="">-- Network Error: Unable to load channels --</option>';
...
async function loadProducts() {
  try {
    ...
    showToast('Unable to load category data. Please check your internet connection.', 'warning', 'Data Loading');
    elements.productsContainer.innerHTML = `
      <div class="error-state" style="text-align: center; padding: 2rem; color: var(--text-secondary);">
        ...
        <p>Unable to load category data. Please check your internet connection and refresh the page.</p>
...
function previewOrder() {
  ...
  if (orderData.length === 0) {
    showToast('No categories selected for preview', 'warning');
    return;
  }
  ...
  previewContent.innerHTML = `
    ...
    <div class="preview-categories">
      <h4>Categories (${orderData.length})</h4>
...
function submitForm() {
  ...
  if (orderData.length === 0) {
    showToast('Please enter at least one category quantity', 'warning', 'No Categories');
    return;
  }
  ...
  modalSummary.innerHTML = `
    ...
    <ul>
      ...
      <li>Categories: ${orderData.length} items</li>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Force remove loading overlay after 3 seconds regardless of loading status
  ...
  try {
    ...
    // Load dropdown data and products in parallel with timeout
    ...
    // Clear force timeout and hide loading overlay
    ...
    if (hasErrors) {
      showToast('Application loaded with limited data. Some features may not work properly.', 'warning', 'Partial Load');
    } else {
    ...
    showToast('Application is ready. Some data may need to be refreshed.', 'warning', 'Partial Load');
  }
});
